...
A controlled comparison of post-training methods (SFT, DPO, KTO/ORPO, RLHF/PPO, GRPO/RLVR) on **one** small base model (Qwen2.5-0.5B-Instruct) with **one** evaluation harness. The comparison — not any individual method — is the contribution. See [PROJECT.md](PROJECT.md) for the full plan-of-record (phases, datasets per phase, compute budget, anti-goals).
